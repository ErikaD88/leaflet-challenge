...
.then(function(plateData) {
    ...
    // load data using geoJson and add to this tectonic plates layer group
    L.geoJson(plateData, {
...
legend.onAdd = function() {
    ...
    // Loop through the depth intervals to create legend labels
    for (let i = 0; i < intervals.length; i++) {
        div.innerHTML +=
            `<i style="background: ${colors[i]}; width: 20px; height: 20px; display: inline-block;"></i> ` +
            intervals[i] + (intervals[i + 1] ? ` – ${intervals[i + 1]} km<br>` : "+ km");
    }
